...
void Enqueue::visit(StmtMatmulAuto& s)
{
    if (_failure) return;
    ...
    const int devNum = _memMgr.deviceNum();
    const double threshold = OCLhacks::singleton().paranoidError( devNum );
    ...
    if (s.isMATMUL())
    {
        ...
        if (OCLhacks::singleton().supportEvergreen( _memMgr.deviceNum() ))
        {
            ...
            const size_t vlen0 = OCLhacks::singleton().supportImages(devNum)
                                     ? 0  // use images
                                     : 1; // no image support
