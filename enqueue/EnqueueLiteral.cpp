...
void Enqueue::visit(StmtLiteral& s)
{
    if (_failure) return;
    ...
    // if stream context dimensions are smaller than LHS variable need a loop
    const bool streamSmallerThanLHS
        = !_subs.top()->isGlobalIndexTiles() &&
          ( _enqueueWidth * _enqueueHeight
                < (lhsWidth / _lhsVecLength) * lhsHeight );

    // check if a loop is necessary over the stream in the statement
    const bool needLoop = everyStmtNeedsLoop || streamSmallerThanLHS;
    ...
    _subs.top()->setVariable( lhsWidth,
                              lhsHeight,
                              getVecLength(s.lhsVariable()) );

    // get literal value to assign
    double rhsVal;
    if (s.litdataPtr())
    {
        switch (s.litdataPtr()->prec())
        {
