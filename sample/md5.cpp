...
int main(int argc, char *argv[])
{
    ...
    // Note:
    // The reference RSA implementation is more efficient and interleaves
    // input data preparation with hashing each 512 bit block. Implementing
    // separate stages is easier to understand.
    ...
    // vector of cleartext (should be multiple of 4 in length)
    textBuf.push_back("first cleartext message");
    textBuf.push_back("second cleartext message");
    textBuf.push_back("third cleartext message");
    textBuf.push_back("fourth cleartext message");
    textBuf.push_back("fifth cleartext message");
    textBuf.push_back("sixth cleartext message");
    textBuf.push_back("seventh cleartext message");
    textBuf.push_back("eighth cleartext message");
    for (size_t i = 0; i < 20; i++)
    {
        stringstream ss;
        ss << "cleartext message number " << i;
        textBuf.push_back(ss.str());
    }
    ...
    {
        cout << "\"" << textBuf[i] << "\" has MD5 hash ";

        const uint8_t *ptrDigest
            = reinterpret_cast<const uint8_t*>( &digest[i][0] );
