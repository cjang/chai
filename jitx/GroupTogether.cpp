...
bool GroupTogether::VariableDesc::isSameDataAcrossTraces(void) const
{
...
size_t GroupTogether::VariableDesc::precision(void) const
{
...
size_t GroupTogether::precision(const uint32_t variable) const
{
    if (_traceVarDesc.count(variable))
    {
        return const_cast< GroupTogether* >( this )
...
size_t GroupTogether::precision(const AstVariable* variable) const
{
    if (variable->isTraceVariable())
    {
...
set< uint32_t > GroupTogether::internalVars(void) const
{
    ...
    for (set< uint32_t >::const_iterator
         it = _constructorLHS.begin();
         it != _constructorLHS.end();
         it++)
    {
