...
void UseRegister::updateRW(const AstVariable* stmtLHS)
{
    ...
    for (set< const AstVariable* >::const_iterator
         it = _stmtRHS.begin(); it != _stmtRHS.end(); it++)
    {
    ...
    for (set< uint32_t >::const_iterator
         it = traceVars.begin(); it != traceVars.end(); it++)
    {
...
void UseRegister::edit(StmtIdSpace& xid)
{
    ...
    for (map< uint32_t, size_t >::const_iterator
         it = _traceRead.begin(); it != _traceRead.end(); it++)
    {
    ...
    for (set< uint32_t >::const_iterator
         it = _traceExclude.begin(); it != _traceExclude.end(); it++)
    {
    ...
    for (set< const StmtBarrier* >::const_iterator
         it = _trackBarriers.begin(); it != _trackBarriers.end(); it++)
    {
        ...
        // trace variables
        for (set< uint32_t >::const_iterator
             jt = sb->traceVars().begin(); jt != sb->traceVars().end(); jt++)
        {
    ...
    for (set< uint32_t >::const_iterator
         it = traceCandidates.begin(); it != traceCandidates.end(); it++)
    {
        const uint32_t varNum = *it;
