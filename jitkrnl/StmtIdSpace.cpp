...
StmtIdSpace::StmtIdSpace(const GroupTogether& together,
    ...
    : _together(together),
      ...
      _scalarVecLength(together.scalarVecLength()),
...
StmtIdSpace::~StmtIdSpace(void)
{
    ...
    for (set< Stmt* >::const_iterator
         it = uniqueStmts.begin(); it != uniqueStmts.end(); it++)
    {
        delete *it;
    }
}
...
bool StmtIdSpace::scalarVecLength(void) const
{
...
size_t StmtIdSpace::getPrec(const uint32_t variable) const
{
    return _together.prec(variable);
}

size_t StmtIdSpace::getPrec(const AstVariable* variable) const
{
    return _together.prec(variable);
}
...
bool StmtIdSpace::randomness(void) const
{
    return false;
}
