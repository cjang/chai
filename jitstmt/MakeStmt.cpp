...
size_t MakeStmt::getTransposeCount(void) const
{
    ...
    for (vector< AstTranspose* >::const_iterator
         it = _transposeStack.begin(); it != _transposeStack.end(); it++)
    {
...
void MakeStmt::specialTrackVar(void)
{
    ...
    {
        _scalarVecLength = true; // force vector length to 1
    }
}
...
void MakeStmt::pushLexScope(Stmt* s)
{
    ...
    s->scalarVecLength(_scalarVecLength);
}
                        ...
                        const SplitContext force)
{
            ...
            )
    {
        ...
        {
            // create new variable
            newVar = new AstVariable( v.getArg(argIndex), 0 /*dummy*/ );
