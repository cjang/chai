...
class JustInTimeMemo
{
    ...
    size_t lookupVectorLength(const VarKey, const bool isDouble);
...
public:
    ...
    bool autotuneVars(Evergreen::MatmulBase&,
                      const XStmtMatmulAuto&,
                      const XStmtIdSpace&);

    bool createArrayTemp(const AstVariable*, XStmtIdSpace&);
    bool createArrayTemp(const AstVariable*, XStmtIdSpace&, const float);
    bool createArrayTemp(const AstVariable*, XStmtIdSpace&, const double);
    bool createArraySendData(const AstVariable*, XStmtIdSpace&, BackMem*);
};
